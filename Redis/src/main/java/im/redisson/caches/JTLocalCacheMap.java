...
public class JTLocalCacheMap extends JTLocalCache implements JTICacheMap
{
    private RMap<Object, Object> cacheMap = null;

    public JTLocalCacheMap(String name)
    {
        super(name);
    ...
    public static JTLocalCacheMap instance(String name)
    {
        JTLocalCacheMap cache = (JTLocalCacheMap) JTCacheManager.getValue(name);
        if (cache == null)
        {
            cache = new JTLocalCacheMap(name);
